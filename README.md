...
```
[extract_IMG_IMU_from_bag.py](..%2F..%2FNetBeansProjects%2FScripts_for_SLAM%2Fextract_IMG_IMU_from_bag.py)
...
# Use the model
model.train(data="coco128.yaml", epochs=3, batch=100)  # train the model
metrics = model.val()  # evaluate model performance on the validation set
results = model("https://ultralytics.com/images/bus.jpg")  # predict on an image
path = model.export(format="onnx")  # export the model to ONNX format
```

#### Use YOLOv8 for Lidar Segmentation Task

YOLOv8 may be used directly in the Command Line Interface (CLI) with a `yolo` command:

```bash
# training
python ./ultralytics/yolo/v8/segment/train.py --data ./ultralytics/datasets/lidar.yaml --model ./ultralytics/models/v8/yolov8m-seg-lidar.yaml --epochs 150 --imgsz 256 --batch 256 --name multiscale
yolo segment train data=./ultralytics/datasets/lidar.yaml model=./ultralytics/models/v8/yolov8m-seg-lidar.yaml epochs=250 imgsz=256 batch=32 name="new_seg"
yolo segment train data=./ultralytics/datasets/lidar.yaml model=./ultralytics/models/v8/yolov8m-seg-lidar.yaml epochs=250 imgsz=256 batch=64 name="new_seg" pretrained=./runs/segment/seg_maya_dataset_aguada_platform/weights/best.pt
yolo segment train data=./ultralytics/datasets/lidar.yaml model=./ultralytics/models/v8/yolov8m-seg-lidar.yaml epochs=250 imgsz=256 batch=32 name="new_seg" devices=[0, 1]

# testing
yolo segment val model=./runs/segment/new_bb_seg_256_x/weights/best.pt name="new_bb_seg_256_x_test" data=./ultralytics/datasets/lidar.yaml imgsz=256 split='test'
python ./ultralytics/yolo/v8/segment/val.py --data ./ultralytics/datasets/lidar.yaml --model ./runs/segment/multiscale_v6_SLOPE/weights/best.pt --imgsz 256 --name multiscale_v6_SLOPE_test --split test


# inference
python ./ultralytics/yolo/v8/segment/predict_lidar.py --model ./runs/segment/train_MLS_SAY_15_aug_per_region/weights/best.pt --classes 0 --imgsz 256 --conf-thres 0.25
```
...
For YOLOv8 bug reports and feature requests please visit [GitHub Issues](https://github.com/ultralytics/ultralytics/issues), and join our [Discord](https://discord.gg/bNc8wwx7Qy) community for questions and discussions!
